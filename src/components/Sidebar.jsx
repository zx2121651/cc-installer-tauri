...
export default function Sidebar({ activeTab, setActiveTab, tabs }) {
  return (
    <div className="w-[64px] lg:w-[220px] bg-[#FFFBF9] border-r border-[#FEEFE6] flex flex-col shrink-0 transition-[width,background-color] duration-300">
      <nav aria-label="主导航" className="flex-1 overflow-y-auto p-2 lg:p-3 space-y-1 custom-scrollbar">
        {tabs.map(tab => {
          const Icon = tab.icon;
      ...
      <div className="hidden lg:block h-[160px] relative shrink-0 overflow-hidden border-t border-[#FEEFE6]">
        ...
        <svg viewBox="0 0 200 150" className="w-full h-full relative z-10 scale-[1.15] translate-y-3 translate-x-2" aria-hidden="true">
          {/* Plant pot */}
