...
import { ListTree, ChevronRight, Globe, Cpu, Archive, Terminal, Command, Sliders, Loader2 } from 'lucide-react';
    ...
    <div className="flex-1 bg-white border border-[#FDECE2] rounded-[20px] p-5 shadow-[0_2px_10px_rgba(0,0,0,0.02)] card-hover-effect">
      <div className="flex justify-between items-center mb-4">
        ...
        <button
          ...
          className="text-[11px] font-bold text-[#F37042] hover:text-[#E06030] flex items-center gap-0.5 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {isCheckingEnv ? (
            <>
      ...
      <div className="space-y-4">
        {[
          ...
          { icon: Sliders, color: 'text-purple-600', title: '执行策略', val: envData.powershell_policy || '未知', status: envData.powershell_policy && envData.powershell_policy !== 'Restricted' ? '正常' : '受限', ok: !!(envData.powershell_policy && envData.powershell_policy !== 'Restricted') },
        ].map((item, i) => {
