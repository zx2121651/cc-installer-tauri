...
import { ShieldCheck, RefreshCw, Cpu, Terminal, CheckCircle2, AlertTriangle, List, Shield } from 'lucide-react';
...
export default function EnvTab({ envData, checkEnv, isCheckingEnv, triggerTool }) {
  const envItems = [
    ...
    { name: 'npm 包管理包', value: envData.npm_version, desc: '安装管理全局及项目内第三方依赖包', rec: '>=9.0.0' },
            ...
            <div key={i} className="border border-[#FDECE2] rounded-2xl p-4 bg-[#FFFBF9] flex flex-col justify-between">
              <div>
                <div className="flex justify-between items-start mb-2">
                  ...
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${isOk ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-500'}`}>
                    {isOk ? '✅ 准备就绪' : '⚠️ 状态异常'}
                  </span>
                </div>
