...
impl PlatformAdapter for MacosAdapter {
    ...
    fn install_node(
        ...
        let _ = log_tx.send("🍏 检测到 macOS 平台。正在准备安装 Node.js...\n".to_string());
        let _ = log_tx.send("⚠️ 提示：macOS/Linux 暂行快速静默开发版，已跳过二进制包下载。\n".to_string());
    ...
    fn fix_execution_policy(&self) -> Result<(), String> {
        // Unix has no powershell execution policy by default
        Ok(())
    }
    ...
    fn add_to_path(&self, target_path: &str) -> Result<(), String> {
        ...
        let mut file = std::fs::OpenOptions::new()
            ...
            .append(true)
            .open(&profile)
            .map_err(|e| format!("无法打开 shell 配置文件: {}", e))?;
    ...
    fn create_desktop_shortcut(&self) -> Result<(), String> {
        ...
        if desktop.exists() {
            let shortcut_file = desktop.join("Claude-Code.command");
            let content = "#!/bin/bash\nopen -a Terminal \"$(which claude)\"\n";
            std::fs::write(&shortcut_file, content).map_err(|e| format!("创建 macOS 桌面快捷脚本失败: {}", e))?;
    ...
    fn uninstall_node(&self, log_tx: Sender<String>) -> Result<(), String> {
        let _ = log_tx.send("🧹 macOS: 正在清理本地绿色 Node.js 目录...\n".to_string());
    ...
    fn uninstall_claude_code(&self, log_tx: Sender<String>) -> Result<(), String> {
        let _ = log_tx.send("🧹 macOS: 正在通过 npm 卸载 Claude Code...\n".to_string());
